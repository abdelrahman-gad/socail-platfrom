  ...
  <div class="bg-white p-4 rounded-lg shadow animate-fade-in-down">
    <div class="flex justify-between border-b-2 pb-3">
      <div class="flex items-center">
        ...
        <select @change="getPosts(null)" v-model="perPage"
        ...
        <span class="ml-3">Found {{ posts?.total }} posts</span>
      </div>
      <div>
        <input v-model="search" @change="getPosts(null)"
    ...
    <table class="table-auto w-full">
      ...
      <tbody v-if="posts.loading || !posts.data.length">
      <tr>
        <td colspan="6">
          <Spinner v-if="posts.loading"/>
      ...
      <tbody v-else>
      <tr v-for="(post, index) of posts.data">
        <td class="border-b p-2 ">{{ post.id }}</td>
        <td class="border-b p-2 ">
          <img v-if="post.image_url" class="w-16 h-16 object-cover" :src="post.image_url" :alt="post.title">
        ...
        <td class="border-b p-2">
          {{ $filters.currencyUSD(post.price) }}
        </td>
        <td class="border-b p-2">
          {{ post.quantity }}
        </td>
              ...
              >
                <div class="px-1 py-1">
                  <MenuItem v-slot="{ active }">
                    <router-link
                      :to="{name: 'app.posts.edit', params: {id: post.id}}"
                      :class="[
    ...
    <div v-if="!posts.loading" class="flex justify-between items-center mt-5">
      <div v-if="posts.data.length">
        Showing from {{ posts.from }} to {{ posts.to }}
      </div>
      <nav
...
const perPage = ref(POSTS_PER_PAGE);
const search = ref('');
